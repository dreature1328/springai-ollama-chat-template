...
本项目是针对大语言模型对话场景的模板工程，基于 Spring AI + Ollama，提供智能对话的解决方案，支持**人格模拟对话、检索增强生成（RAG）、对话历史存储**，便于初学者快速搭建应用。
...
| 存储类型              | 主要用途                           |
| :-------------------- | :--------------------------------- |
| PostgreSQL + pgvector | 人格设定、文档向量、对话历史持久化 |
| Redis                 | 对话上下文缓存                     |
...
已取消 `ChatClient` 接收参数 `MessageChatMemoryAdvisor` （二者框架提供）附带的自动记忆对话功能，而在相应服务实现。
